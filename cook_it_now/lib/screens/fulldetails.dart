...
class _RecipeDetailsPageState extends State<RecipeDetailsPage> {
  ...
  RecipeItem get recipe => widget.recipe;
  ...
  Future<void> _checkIfSaved() async {
    try {
      ...
      if (email == null || email.isEmpty || recipe.id == null) {
        if (!mounted) return;
        setState(() {
          _isSaved = false;
      ...
      final saved = await _savedRecipeService.isRecipeSaved(email, recipe.id!);
  ...
  Future<void> _toggleSaveRecipe() async {
    if (_isSaving || recipe.id == null) return;
    ...
    try {
      if (_isSaved) {
        await _savedRecipeService.removeRecipe(email, recipe.id!);
        ...
        await _savedRecipeService.saveRecipe(email, recipe.id!);
  ...
  Widget _imageSection() {
    if (recipe.url == null || recipe.url!.trim().isEmpty) {
      return _emptyImage();
    }

    return FutureBuilder<String?>(
      future: NlpService.getRecipeImage(recipe.url!),
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: IconButton(
              onPressed: (_isSaving || _isCheckingSaved || recipe.id == null)
                  ? null
                  : _toggleSaveRecipe,
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _imageSection(),
                const SizedBox(height: 12),
                _detailTile("Record Health", recipe.recordHealth),
                _detailTile("Rating", recipe.rating?.toString()),
                _detailTile("Description", recipe.description),
                _detailTile("Cuisine", recipe.cuisine),
                _detailTile("Diet", recipe.diet),
                _detailTile("Prep Time", recipe.prepTime),
                _detailTile("Cook Time", recipe.cookTime),
                _detailTile("Ingredients", recipe.ingredients),
                _detailTile("Instructions", recipe.instructions),
                 _buildAiSection(),
              ],
            ),
          ),
