...
class _VerificationScreenState extends State<VerificationScreen> {
  ...
  Future<void> verifyOtp() async {
    ...
    final result = await EmailAuthService.verifySignupOtp(
      email: widget.email,
      otpCode: otp,
    );
    ...
    if (result["success"] == true) {
      await SessionService.saveEmail(widget.email);

      if (!mounted) return;
      context.go(
        '/firstpage',
        extra: {'email': widget.email, 'isNewUser': true},
      );
    } else {
      setState(() {
  ...
  Future<void> resendOtp() async {
    ...
    final result = await EmailAuthService.resendSignupOtp(email: widget.email);
